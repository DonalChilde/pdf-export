"""conftest.py file for pdfexport"""
import json
import logging
...
from pdfexport.app_lib.logging import rotating_file_logger
...
def json_resources_example_(logger: logging.Logger) -> Dict[str, FileResource]:
    ...
    """
    json_resources = {}
    resource_path: str = "tests.pdfexport.resources.example.json_resources"
...
def env_setup(monkeypatch, test_app_data_dir):
    ...
    monkeypatch.setenv(
        "PFMSOFT_PDFEXPORT_TESTING",
        "True",
    )
    monkeypatch.setenv(
        "PFMSOFT_PDFEXPORT_LOG_LEVEL",
        str(APP_LOG_LEVEL),
    )
    monkeypatch.setenv(
        "PFMSOFT_PDFEXPORT_APP_DIR",
